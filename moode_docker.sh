...
case $yn in 
	[yY] ) echo ok, we will proceed;
        sudo systemctl stop mpd.service mpd.socket nfs-client.target smbd.service rcpbind.service
	sudo systemctl disable mpd.service mpd.socket nfs-client.target smbd.service rcpbind.service
	sudo systemctl mask mpd.service mpd.socket nfs-client.target smbd.service rcpbind.service
		break;;
	[nN] ) echo exiting...;
		break;;
	* ) echo invalid response;;
...
docker create --name debian-moode --restart always -v /sys/fs/cgroup:/sys/fs/cgroup:ro -v /home/moode:/boot:rw -v /home/moode:/var/lib/mpd/:rw --device /dev/snd --net host --privileged -e LANG=C.UTF-8 --cap-add=NET_ADMIN --security-opt seccomp:unconfined --cpu-shares=10240 navikey/raspbian-bullseye /lib/systemd/systemd
...
docker exec -ti debian-moode /bin/bash -c "apt-get install -y curl sudo libxaw7 ssh libsndfile1 libsndfile1-dev dphys-swapfile cifs-utils nfs-common"
